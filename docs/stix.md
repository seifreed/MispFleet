...
Markings are matched case-insensitively, and an attribute's own TLP reaches
its indicator: markings, not `labels`, are what a consumer honours, so a
`tlp:red` attribute inside a `tlp:green` event carries both.

STIX 2.1 fixes exactly four TLP definitions, so `tlp:amber+strict` is exported
as **TLP:AMBER**. That is a deliberate, documented widening: AMBER permits
sharing with clients where AMBER+STRICT does not. The alternative — emitting
no marking, as before — published the most restricted level as an entirely
unrestricted bundle, so the narrower assertion is the safer of the two. Do not
rely on a STIX 2.1 bundle to carry AMBER+STRICT.
