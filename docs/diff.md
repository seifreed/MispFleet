...
The result includes a summary (`added`, `removed`, `changed`, `conflicts`),
`equivalent: true` when the normalized content matches exactly, and a
deterministic `similarity` score (0–1) so near-duplicates are visible even
when the events differ. Volatile
