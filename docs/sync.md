...
during apply. A plan with blocking errors is refused before anything is
applied, with or without `--dry-run`, so `sync run` never leaves the
unblocked half of a rejected job applied.
