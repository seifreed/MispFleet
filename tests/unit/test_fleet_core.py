...
from mispfleet.fleet.fleet import _sighting_count
...
def test_sighting_count_handles_every_misp_response_shape() -> None:
    from mispfleet.fleet.fleet import _sighting_count

    eq(_sighting_count({"Sighting": {"id": "1"}}), 1)
    eq(_sighting_count({"Sighting": [{"id": "1"}, {"id": "2"}]}), 2)
    eq(_sighting_count({"message": "3 sighting(s) added."}), 3)
    eq(_sighting_count({"message": "nothing matched"}), 0)
    eq(_sighting_count({}), 0)


def test_sighting_count_reads_the_shape_the_spec_declares() -> None:
    """openapi.yaml declares SightingResponse as the Sighting object itself.

    Counting only a "Sighting" envelope reported 0 created against a
    conforming server, so `sightings push` exited 11 for a sighting that had
    in fact landed. Scanning the message for any digit run also read the 203
    out of "No match for 203.0.113.10" as a count.
    """
    flat = {"id": "1", "uuid": "u", "attribute_uuid": "a-1", "event_id": "7"}
    eq(_sighting_count(flat), 1)
    eq(_sighting_count({"Sighting": flat}), 1)
    eq(_sighting_count({"Sighting": [flat, flat]}), 2)
    eq(_sighting_count({"message": "2 sighting(s) added."}), 2)
    eq(_sighting_count({"message": "12 sightings added for 3 attributes"}), 12)
    eq(_sighting_count({"message": "No match for 203.0.113.10"}), 0)
    eq(_sighting_count({}), 0)
