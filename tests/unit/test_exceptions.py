...
def test_connection_error_alias_is_the_same_class() -> None:
    ok(ConnectionFailedError is ConnectionError)
    ok(issubclass(ConnectionError, TransportError))
    ok(not issubclass(ConnectionError, OSError))
    error = ConnectionFailedError("unreachable")
    ok(isinstance(error, ConnectionError))
    eq(type(error).__name__, "ConnectionError")
