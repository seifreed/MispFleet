...
async def test_fleet_overhead_stays_close_to_direct_requests(loaded_misp: FakeMisp) -> None:
    ...
    async with fleet:
        ...
        # Scheduling noise only ever adds time, so the minimum of interleaved
        # samples is the stable estimate. Comparing one sample of each made
        # this gate fail whenever the machine was busy.
        for _ in range(5):
            direct_samples.append(await direct_stream(config, rounds=rounds))
            fleet_samples.append(await fleet_stream(fleet, rounds=rounds))
    direct = min(direct_samples)
    fleet_elapsed = min(fleet_samples)
