...
async def test_health_never_claims_a_valid_certificate_without_verification(
    tls_fake_misp: FakeMisp,
) -> None:
    """With verify_tls off nothing validated the certificate.

    Reporting tls_valid=True there asserted a security property the run never
    checked; the honest answer is unknown.
    """
    config = config_for(tls_fake_misp, verify_tls=False)
    async with MispClient(config, api_key=API_KEY) as client:
        health = await check_server(client)
    ok(health.reachable)
    eq(health.tls_valid, None)
    contains(health.warnings[0], "TLS verification is disabled")
...
async def test_search_and_streaming_record_query_fingerprints(fake_misp: FakeMisp) -> None:
    from mispfleet.models.query import SearchQuery

    fake_misp.attributes = [{"id": "1", "type": "domain", "value": "evil.example"}]
    backend = MemoryStateBackend()
    fleet = fleet_with_state({"alpha": config_for(fake_misp, name="alpha")}, backend)
    query = SearchQuery(value="evil.example")
    async with fleet:
        await fleet.search(query)
        streamed = [match async for match in fleet.iter_search(query)]
    eq(len(streamed), 1)
    records = await backend.list_queries()
    eq(len(records), 2)
    eq({record.fingerprint for record in records}, {query.fingerprint()})
    eq({record.operation for record in records}, {"federated-search", "federated-stream"})
    eq({record.servers[0] for record in records}, {"alpha"})
    ok(all(record.record_count >= 1 for record in records))
