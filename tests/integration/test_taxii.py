...
async def test_null_collection_list_is_an_empty_list(fake_taxii: FakeTaxii) -> None:
    """A present-but-null key made ``list(None)`` raise an untyped TypeError."""
    fake_taxii.collections_body = {"collections": None}
    async with TaxiiClient(fake_taxii.url, token=TOKEN) as client:
        eq(await client.collections(API_ROOT), [])


async def test_non_list_collections_are_typed_response_errors(fake_taxii: FakeTaxii) -> None:
    fake_taxii.collections_body = {"collections": "everything"}
    async with TaxiiClient(fake_taxii.url, token=TOKEN) as client:
        with pytest.raises(InvalidResponseError) as excinfo:
            await client.collections(API_ROOT)
    contains(str(excinfo.value), "collection list")
...
def test_taxii_non_object_responses_raise_typed_errors() -> None:
    """TAXII promises JSON objects; anything else is an invalid response."""
    from mispfleet.exceptions import InvalidResponseError
    from mispfleet.services.taxii import _mapping

    eq(_mapping({"a": 1}, "/x"), {"a": 1})
    with pytest.raises(InvalidResponseError) as raised:
        _mapping(["a", "list"], "/x")
    contains(raised.value.message, "list where an object was expected")
