...
class FleetExecutor:
    ...
    async def run[T](
        ...
        async def run_one(name: str) -> None:
            async with semaphore:
                try:
                    ...
                    # One server answering with unexpected data must not take
                    # the whole fleet down: record it as that server's failure.
                    errors[name] = ServerError(
                        server=name,
                        kind=type(error).__name__,
                        message=f"unexpected failure handling {name}: {error}",
                    )
                    logger.warning("server %s raised %s: %s", name, type(error).__name__, error)
                    if options.failure_policy is FailurePolicy.FAIL_FAST:
                        raise
        ...
        try:
            ...
            # Failures are already recorded per server; fail-fast re-raises are
            # surfaced as PartialFleetError by the policy check below.
            logger.debug("fleet fan-out ended with %d failure(s)", len(group.exceptions))
        finally:
