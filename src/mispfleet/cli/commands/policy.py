...
from mispfleet.cli.context import EXIT_POLICY, EXIT_SUCCESS, EXIT_USAGE, guard, run, state_of
...
def validate(ctx: typer.Context, name: Annotated[str, typer.Argument()]) -> None:
    ...
    def inner() -> None:
        ...
        state.emit(
            ...
            render=lambda console: console.print(f"policy {name!r} is valid"),
        )
...
def test(
    ...
    state = state_of(ctx)
    try:
        event = MISPEvent.from_misp(json.loads(event_file.read_text(encoding="utf-8")))
    ...
    async def inner() -> int:
        ...
        def render(console: Console) -> None:
            ...
            for violation in result.violations:
                console.print(f"  [red]violation[/red]: {violation.rule}: {violation.message}")
