...
async def check_server(client: MispClient) -> ServerHealth:
    ...
    return ServerHealth(
        ...
        # Only a verified handshake proves the certificate: with verification
        # disabled nothing was checked, so the answer is unknown, not True.
        tls_valid=(
            True if client.config.url.scheme == "https" and client.config.verify_tls else None
        ),
