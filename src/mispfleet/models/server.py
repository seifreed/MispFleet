...
CredentialProviderName = Literal["env", "keyring", "prompt", "memory", "op"]
...
class RetryConfig(BaseModel):
    ...
    # A server-supplied Retry-After is honored, but never past this ceiling:
    # one hostile header would otherwise park the client in sleep for years.
    max_retry_after: float = Field(default=300.0, ge=0.0)
